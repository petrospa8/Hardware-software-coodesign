...
/*	sc_in<sc_int<32>>	dataIn;
	...
	mipsCA(sc_module_name name_, char* codeFileName, char* dataFileName) : sc_module(name_) {
		...
		instcontrol->inm(inm);
		instcontrol->lbType(lbType);
		instcontrol->lbDispl(lbDispl);
		...
		lbComp->salidaMem(salidaMem);
		lbComp->displ(lbDispl);
		lbComp->type(lbType);
		lbComp->salidaLb(salidaLb);
	}
...
private:
	...
	sc_signal<sc_int<32>> salidaLb;
	sc_signal<sc_uint<6>> lbType;
	sc_signal<sc_uint<2>> lbDispl;
};
